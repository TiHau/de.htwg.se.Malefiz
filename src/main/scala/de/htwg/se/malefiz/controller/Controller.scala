...
case class Controller(var gameBoard: GameBoard) {
  ...
  def setDicedFields(f1:String): Unit ={
    ...
    if(!gameBoard.board(x1 - diced)(y1).isFreeSpace()){
      gameBoard.board(x1 - diced)(y1).asInstanceOf[Field].avariable=true
    }
    if(!gameBoard.board(x1 + diced)(y1).isFreeSpace()){
      gameBoard.board(x1 + diced)(y1).asInstanceOf[Field].avariable=true
    }
    if(!gameBoard.board(x1)(y1 - diced).isFreeSpace()){
      gameBoard.board(x1)(y1 - diced).asInstanceOf[Field].avariable=true
    }
    if(!gameBoard.board(x1)(y1 + diced).isFreeSpace()){
      gameBoard.board(x1)(y1 + diced).asInstanceOf[Field].avariable=true
    }
  ...
  def unsetDicedFields(f1:String): Unit ={
    ...
    val y1 = (f1.charAt(2).toString + f1.charAt(3)).toInt
    if(!gameBoard.board(x1 - diced)(y1).isFreeSpace()){
      gameBoard.board(x1 - diced)(y1).asInstanceOf[Field].avariable=false
    }
    if(!gameBoard.board(x1 + diced)(y1).isFreeSpace()){
      gameBoard.board(x1 + diced)(y1).asInstanceOf[Field].avariable=false
    }
    if(!gameBoard.board(x1)(y1 - diced).isFreeSpace()){
      gameBoard.board(x1)(y1 - diced).asInstanceOf[Field].avariable=false
    }
    if(!gameBoard.board(x1)(y1 + diced).isFreeSpace()){
      gameBoard.board(x1)(y1 + diced).asInstanceOf[Field].avariable=false
    }
